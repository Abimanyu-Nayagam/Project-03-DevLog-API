...
def test_create_snippet_success(auth_client, snippet_data):
    res = auth_client.post("/api/v1/snippets", json=snippet_data)
...
def test_create_snippet_missing_required(auth_client, snippet_data):
    for field in ["title", "language", "snippet", "description"]:
        ...
        bad_data.pop(field)
        res = auth_client.post("/api/v1/snippets", json=bad_data)
...
def test_create_entry_success(auth_client, entry_data):
    res = auth_client.post("/api/v1/entries", json=entry_data)
    assert res.status_code == 201
    data = res.get_json()
...
def test_create_entry_missing_required(auth_client, entry_data):
    for field in ["title", "content"]:
        ...
        bad_data.pop(field)
        res = auth_client.post("/api/v1/entries", json=bad_data)
        assert res.status_code == 400
...
def test_get_snippets_success(auth_client, snippet_data):
    auth_client.post("/api/v1/snippets", json=snippet_data)
    res = auth_client.get("/api/v1/snippets")
    assert res.status_code == 200
    data = res.get_json()
    assert len(data) >= 1

def test_get_entries_success(auth_client, entry_data):
    auth_client.post("/api/v1/entries", json=entry_data)
    res = auth_client.get("/api/v1/entries")
    assert res.status_code == 200
    data = res.get_json()
    assert len(data) >= 1

def test_get_snippet_by_invalid_id(auth_client):
    res = auth_client.get("/api/v1/snippets/99999")
    assert res.status_code == 404

def test_get_entry_by_invalid_id(auth_client):
    res = auth_client.get("/api/v1/entries/99999")
    assert res.status_code == 404
...
def test_update_snippet_success(auth_client, snippet_data):
    ...
    res2 = auth_client.patch("/api/v1/snippets", json=patch_data)
...
def test_update_entry_success(auth_client, entry_data):
    ...
    res2 = auth_client.patch("/api/v1/entries", json=patch_data)
    assert res2.status_code == 200
...
def test_update_snippet_invalid(auth_client, snippet_data):
    ...
    res2 = auth_client.patch("/api/v1/snippets", json=bad_data)
...
def test_delete_snippet_success(auth_client, snippet_data):
    res = auth_client.post("/api/v1/snippets", json=snippet_data)
    snippet_id = res.get_json()["id"]
    res2 = auth_client.delete(f"/api/v1/snippets/{snippet_id}")
    assert res2.status_code == 200
    res3 = auth_client.get(f"/api/v1/snippets/{snippet_id}")
    assert res3.status_code == 404

def test_delete_entry_success(auth_client, entry_data):
    res = auth_client.post("/api/v1/entries", json=entry_data)
    entry_id = res.get_json()["id"]
    res2 = auth_client.delete(f"/api/v1/entries/{entry_id}")
    assert res2.status_code == 200
    res3 = auth_client.get(f"/api/v1/entries/{entry_id}")
    assert res3.status_code == 404

def test_delete_nonexistent_snippet(auth_client):
    res = auth_client.delete("/api/v1/snippets/9999")
    assert res.status_code == 404

def test_delete_nonexistent_entry(auth_client):
    res = auth_client.delete("/api/v1/entries/9999")
    assert res.status_code == 404
